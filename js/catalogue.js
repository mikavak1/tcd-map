  ...
  Object.keys(groups)
    ...
    .forEach((letter) => {
      ...
      groups[letter].forEach((b) => {
        ...
        entry.innerHTML = `
          <span class="building-entry-name">${b.name}</span>
...
function renderFilterPanel() {
  ...
  Object.entries(CATEGORIES).forEach(([key, cat]) => {
    ...
    item.innerHTML = `
      <div class="filter-item-icon" style="background-color:${cat.colour}20;">
...
function renderSavedPanel() {
  ...
  savedBuildings.forEach((b) => {
    ...
    item.innerHTML = `
      <div class="saved-panel-icon" style="background-color:${cat.colour}20;">
